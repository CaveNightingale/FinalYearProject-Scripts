...
module tb_gemm_ones;
  ...
  task automatic gen_all_ones_xy;
    integer n;
    integer m;
    integer kk;
    begin
      for (n = 0; n < BATCH; n = n + 1) begin
        for (kk = 0; kk < K; kk = kk + 1) begin
          x_real[idx_x(n, kk)] = 1.0;
        end
      end

      for (kk = 0; kk < K; kk = kk + 1) begin
        for (m = 0; m < OUT; m = m + 1) begin
          w_real[idx_w(kk, m)] = 1.0;
        end
      end
    end
  endtask
  ...
  initial begin
    $dumpfile("tb_gemm_ones.vcd");
    $dumpvars(0, tb_gemm_ones);
    trace_fd = $fopen("tb_gemm_ones_trace.txt", "w");
    if (trace_fd == 0) begin
      $display("[FAIL][tb_gemm_ones] cannot open tb_gemm_ones_trace.txt");
      $fatal;
    end
    $fdisplay(trace_fd, "tb_gemm_ones cycle trace");
    ...
    run_case(2'b00);  // e4m3, test 8x64 * 64x32 all-ones

    if (fail_cases == 0) begin
      $display("[PASS] tb_gemm_ones passed all-ones case with tol=%e total_cycles=%0d", TOL, total_cycles);
    end else begin
      $display("[FAIL] tb_gemm_ones fail_cases=%0d total_cycles=%0d", fail_cases, total_cycles);
      $fatal;
    end
